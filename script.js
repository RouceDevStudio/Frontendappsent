...
(function initTerms() {
  ...
  const isFirst=!LS.get('upgrades_onboarding_done');
  if(LS.get('upgames_terms_version')!==TERMS_VER) {
    ...
    btnAccept.addEventListener('click',()=>{
      ...
      document.body.style.overflow='';
      if(isFirst) setTimeout(startTutorial,600);
    });
  }
})();
...
// ── 8. ENVIAR CONTEXTO AL IFRAME CUANDO SE ABRE ─────────
// Al abrir el widget, pasarle el usuario y el estado actual del feed
...
window.nxToggle = function() {
  ...
  if (nxOpen) {
    ...
    // Esperar a que el iframe esté cargado antes de postear
    ...
    // Si ya está cargado, enviar inmediatamente; si no, al load
